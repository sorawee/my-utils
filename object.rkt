...
(require racket/hash
         ...
         "./section.rkt"
         "./macro.rkt"
         (for-syntax (except-in racket/base #%app)
                     fancy-app))
...
(define union-hash (hash-union _ _ #:combine (λ (a b) b)))
      ...
      (syntax-parse items
        [() (cond
              [(check-duplicate-identifier static-ids)
               => (raise-syntax-error 'object "duplicate static field" orig-stx _)]
              [else h])]
        [([{~literal ...} e:expr] . xs) (lp #'xs #`(union-hash #,h e) static-ids)]
        [(item:item-cls . xs)
         (lp #'xs #`(hash-set #,h item.key item.expr)
             (cond
               [(attribute item.static-id) => (cons _ static-ids)]
               [else static-ids]))]))))
